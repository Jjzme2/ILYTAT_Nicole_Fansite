<template>
    <div class="space-y-1.5" :class="$attrs.class" :style="$attrs.style">
        ...
        <div class="relative group">
            ...
            <!-- Icon Right -->
            <div v-if="$slots['icon-right']" class="absolute right-3 top-1/2 -translate-y-1/2 text-muted group-focus-within:text-primary transition-colors">
                <slot name="icon-right" />
            </div>
        </div>

        <p v-if="error" :id="errorId" class="text-xs text-red-500 font-medium ml-1 flex items-center gap-1">
