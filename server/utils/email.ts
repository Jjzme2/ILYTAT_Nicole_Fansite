...
export async function sendEmail(config: any, payload: EmailPayload): Promise<{ success: boolean; message: string; provider?: 'emailjs' | 'nodemailer' }> {
    // Waterfall: Try EmailJS first, then Nodemailer
    let sent = false
    let provider: 'emailjs' | 'nodemailer' | undefined

    // 1. Try EmailJS
    sent = await sendWithEmailJS(config, payload)
    ...
    if (!sent) {
        ...
        sent = await sendWithNodemailer(config, payload)
